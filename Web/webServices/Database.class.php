...
function setLocation($uid,$longitude,$latitude,$altitude,$gravity)
{
    $sql= "SELECT COUNT(iduser) AS Total FROM locations WHERE longitude=".$longitude." AND latitude=".$latitude." AND iduser='".$uid."'";
    ...
    //check if location already exist client
    if ($total==0)
    {
	    $sql = "INSERT INTO locations(iduser,longitude,latitude,altitude,gravity) VALUES('".$uid."',".$longitude.",".$latitude.",".$altitude.",".$gravity.")";	
...
{
	$iduser=$this->getUser($email);
	
    //Check if the user already exist
    if (empty($iduser))
    {
        $sql = "INSERT INTO user(iduser,email,fullname) VALUES(uuid(),'".$email."','".$fullname."')";	
        ...
        //Created User
        {
            ...
            $sql = "INSERT INTO userDetails(iduser) VALUES('".$iduser."')";	
